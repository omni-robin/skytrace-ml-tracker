import threading
import json
import os
import signal
import sys
import atexit
from confluent_kafka import Consumer, Producer
from nicegui import ui, app, run
from sigmf_plotter import SigMFPanoramaPlotter
import subprocess
import plotly.graph_objects as go

# Skytrace branding asset (served as a static file)
ASSET_DIR = '/Users/omni/.openclaw/workspace'
RAVEN_IMG = 'skytrace_raven_8k.png'

# Optional dechirp demo utilities (included in this drop-in)
try:
    from dechirp_tools import (
        make_dechirp_waterfall,
        make_shifted_waterfall,
        cfo_metrics,
        symbol_barcode_metrics,
        barcode_similarity,
        ambiguity_map,
        load_sigmf_meta,
        parse_sigmf_params,
    )
except Exception:
    make_dechirp_waterfall = None
    make_shifted_waterfall = None
    cfo_metrics = None
    symbol_barcode_metrics = None
    barcode_similarity = None
    ambiguity_map = None
    load_sigmf_meta = None
    parse_sigmf_params = None

# Optional thin NGVA adapter (JSON over udp/http/stdout). File is included in the drop-in.
try:
    import ngva_adapter
except Exception:
    ngva_adapter = None
from pydantic import Field
from pydantic_settings import BaseSettings, SettingsConfigDict


class KafkaSettings(BaseSettings):
    bootstrap_servers: str = Field("kafka:29092", alias="KAFKA_BOOTSTRAP_SERVERS")
    group_id: str = Field("local-analysis-group", alias="GROUP_ID")
    topic: str = Field("iq_stream_topic", alias="KAFKA_TOPIC")
    auto_offset_reset: str = "earliest"

    # Optional: run detector + publish results
    detections_topic: str = Field("iq_detections_topic", alias="KAFKA_DETECTIONS_TOPIC")
    enable_detector: bool = Field(False, alias="ENABLE_DETECTOR")
    detector_cmd: str = Field("edgehub-lora-detector", alias="DETECTOR_CMD")

    # Extra args passed verbatim to the detector (advanced tuning)
    # Example:
    #   DETECTOR_ARGS="--fft-size 2048 --hop 256 --threshold-db 6"
    detector_args: str = Field("", alias="DETECTOR_ARGS")

    # Optional preset name that expands to a known-good set of args.
    # This keeps defaults conservative while enabling easy overrides.
    detector_preset: str = Field("", alias="DETECTOR_PRESET")


class PlotterSettings(BaseSettings):
    nfft: int = 8192
    window: str = "hann"
    port: int = 8080


class AppSettings(BaseSettings):
    # Load from a .env file if it exists
    model_config = SettingsConfigDict(env_file=".env", env_nested_delimiter="__")

    kafka: KafkaSettings = KafkaSettings()
    plotter: PlotterSettings = PlotterSettings()


# Global instance to be used across the module
settings = AppSettings()

# Global shutdown event for graceful termination
shutdown_event = threading.Event()

# Optional Kafka producer for detections
_producer_ref = None

# Latest detections per base_path (for UI exploration)
_latest_det_by_base: dict[str, dict] = {}
_latest_det_lock = threading.Lock()

# Global barcode cache for compare UI (persists across page reloads/reconnects)
_BARCODE_CACHE: dict[str, dict] = {}
_BARCODE_CACHE_LOCK = threading.Lock()

# Global consumer reference for cleanup
_consumer_ref = None
_consumer_thread_ref = None
_cleanup_done = threading.Lock()
_cleanup_started = False

def kafka_worker(plotter):
    global _consumer_ref, _producer_ref

    # Map Pydantic fields to the format confluent-kafka expects
    kafka_conf = {
        "bootstrap.servers": settings.kafka.bootstrap_servers,
        "group.id": settings.kafka.group_id,
        "auto.offset.reset": settings.kafka.auto_offset_reset,
        # Enable auto-commit for cleaner shutdown
        "enable.auto.commit": True,
        "auto.commit.interval.ms": 1000,
        # Reduce session timeout so Kafka detects failures faster
        "session.timeout.ms": 6000,
        "heartbeat.interval.ms": 2000,
        # Ensure we close cleanly on timeout
        "enable.auto.offset.store": True,
    }

    consumer = Consumer(kafka_conf)
    _consumer_ref = consumer  # Store global reference for cleanup
    consumer.subscribe([settings.kafka.topic])

    producer = None
    if settings.kafka.enable_detector:
        producer = Producer({"bootstrap.servers": settings.kafka.bootstrap_servers})
        _producer_ref = producer
        print(f"Detector enabled; publishing to {settings.kafka.detections_topic}")
        print(f"Detector command: {settings.kafka.detector_cmd}")

    print(f"Watching {settings.kafka.topic} on {settings.kafka.bootstrap_servers}...")
    print(f"Consumer group: {settings.kafka.group_id}")
    try:
        while not shutdown_event.is_set():
            try:
                msg = consumer.poll(1.0)
                if msg is None:
                    continue
                if msg.error():
                    print(f"Consumer error: {msg.error()}")
                    continue

                event_data = json.loads(msg.value().decode("utf-8"))
                meta_path = event_data.get("meta_file_path")
                data_path = event_data.get("data_file_path")

                # Process the IQ data and update the web UI
                if os.path.exists(meta_path) and os.path.exists(data_path):
                    base_path = meta_path[:-11]  # Remove ".sigmf-meta" suffix
                    try:
                        plotter.add_base(base_path)
                    except Exception as add_error:
                        print(f"Error processing {base_path}: {add_error}")
                        import traceback
                        traceback.print_exc()

                    # Optional: run detector and publish results back into Kafka
                    if producer is not None:
                        try:
                            det = run_detector(meta_path, data_path)
                            det_event = build_detection_event(event_data, meta_path, data_path, base_path, det)
                            # cache for UI
                            with _latest_det_lock:
                                _latest_det_by_base[base_path] = det_event
                            producer.produce(
                                settings.kafka.detections_topic,
                                json.dumps(det_event).encode("utf-8"),
                            )
                            producer.poll(0)

                            # Optional: forward to NGVA Data Bus adapter (thin bridge)
                            if os.getenv("ENABLE_NGVA_ADAPTER", "0") in ("1", "true", "TRUE", "yes", "YES"):
                                if ngva_adapter is None:
                                    print("NGVA adapter enabled but ngva_adapter.py not available")
                                else:
                                    try:
                                        ngva_adapter.send(det_event)
                                    except Exception as ngva_err:
                                        print(f"NGVA adapter error: {ngva_err}")
                        except Exception as det_err:
                            print(f"Detector error for {base_path}: {det_err}")
                            import traceback
                            traceback.print_exc()
                else:
                    print(f"Warning: Files not found - meta: {meta_path}, data: {data_path}")
            except Exception as loop_error:
                print(f"Error in message loop: {loop_error}")
                import traceback
                traceback.print_exc()
                # Continue the loop even if one message fails
                continue
    except Exception as e:
        print(f"Worker Error: {e}")
    finally:
        print("Kafka worker shutting down gracefully...")
        try:
            if producer is not None:
                try:
                    producer.flush(2.0)
                except Exception as flush_error:
                    print(f"Error flushing producer: {flush_error}")
            consumer.close()
            print("Kafka consumer closed.")
        except Exception as close_error:
            print(f"Error closing consumer: {close_error}")
        finally:
            _consumer_ref = None
            _producer_ref = None


def _utc_now_iso() -> str:
    from datetime import datetime, timezone
    return datetime.now(timezone.utc).isoformat()


def detector_version() -> str:
    """Best-effort detector version string."""
    try:
        res = subprocess.run([settings.kafka.detector_cmd, "--version"], capture_output=True, text=True)
        if res.returncode == 0 and res.stdout.strip():
            return res.stdout.strip()
    except Exception:
        pass
    return "unknown"


def run_detector(meta_path: str, data_path: str) -> dict:
    """Run edgehub-lora-detector and return parsed JSON."""
    import shlex

    # Presets are only applied when explicitly selected.
    presets: dict[str, list[str]] = {
        # High-sample-rate / short-symbol captures (e.g. SF5-ish). More overlap.
        # Note: includes permissive gates for iteration; tighten later as we stabilize.
        "high_sr_sf5": [
            "--fft-size",
            "1024",
            "--hop",
            "128",
            "--threshold-db",
            "8",
            "--min-area",
            "20",
            "--min-height-px",
            "2",
            "--min-width-px",
            "1",
            "--min-burst-snr-db",
            "0",
            "--group-min-pulse-count",
            "1",
            "--group-min-avg-snr-db",
            "0",
            "--emit-non-lora",
        ],

        # Rapid scan mode: useful as a cheap "something is here" detector.
        # Emits peak bands rather than packet slices.
        "rapid_scan": [
            "--mode",
            "rapid",
            "--fft-size",
            "8192",
            "--rapid-windows",
            "32",
            "--threshold-db",
            "8",
        ],
    }

    extra_args: list[str] = []

    if settings.kafka.detector_preset:
        preset = settings.kafka.detector_preset.strip()
        if preset not in presets:
            raise ValueError(f"Unknown DETECTOR_PRESET={preset!r}; available: {sorted(presets.keys())}")
        extra_args.extend(presets[preset])

    if settings.kafka.detector_args:
        # shlex to support quoted strings and avoid naive .split()
        extra_args.extend(shlex.split(settings.kafka.detector_args))

    cmd = [
        settings.kafka.detector_cmd,
        "--meta",
        meta_path,
        "--data",
        data_path,
        *extra_args,
    ]

    # Keep stderr so we can debug failures
    res = subprocess.run(cmd, capture_output=True, text=True)
    if res.returncode != 0:
        raise RuntimeError(
            f"detector failed (code={res.returncode}) stdout={res.stdout[-4000:]} stderr={res.stderr[-4000:]} cmd={cmd}"
        )
    return json.loads(res.stdout)


def build_detection_event(source_event: dict, meta_path: str, data_path: str, base_path: str, det: dict) -> dict:
    # Envelope to keep provenance + allow schema evolution.
    return {
        "event_type": "detections_computed",
        "schema_version": 1,
        "computed_at": _utc_now_iso(),
        "source_event": source_event,
        "artifact": {
            "base_path": base_path,
            "meta_file_path": meta_path,
            "data_file_path": data_path,
        },
        "detector": {
            "name": "edgehub-lora-detector",
            "cmd": settings.kafka.detector_cmd,
            "version": detector_version(),
        },
        "detections": det,
    }


def cleanup_consumer():
    """Cleanup function to close Kafka consumer gracefully"""
    global _cleanup_started

    # Prevent multiple invocations
    with _cleanup_done:
        if _cleanup_started:
            print("Cleanup already in progress, skipping...")
            return
        _cleanup_started = True

    print("Cleanup handler triggered...")
    shutdown_event.set()

    # Don't wait for thread, just signal it to stop
    # The thread will close the consumer itself in the finally block
    print("Signaled Kafka worker to stop...")

    # Give a brief moment for the worker to exit naturally
    # but don't block indefinitely
    if _consumer_thread_ref and _consumer_thread_ref.is_alive():
        _consumer_thread_ref.join(timeout=2)
        if _consumer_thread_ref.is_alive():
            print("Worker still running after 2s, proceeding with shutdown...")


def run_local_consumer():
    global _consumer_thread_ref

    # Serve static assets (logo, etc.)
    try:
        app.add_static_files('/assets', ASSET_DIR)
    except Exception:
        # Safe to ignore if already registered
        pass

    # Initialize using validated settings
    plotter = SigMFPanoramaPlotter(
        nfft=settings.plotter.nfft, window=settings.plotter.window
    )

    # Start Kafka in a background thread as daemon.
    # For local UI-only testing (no Kafka available), set DISABLE_KAFKA=true.
    if os.getenv("DISABLE_KAFKA", "0") in ("1", "true", "TRUE", "yes", "YES"):
        print("DISABLE_KAFKA=true -> Kafka worker not started (UI-only mode)")
    else:
        # Daemon threads don't block process exit
        thread = threading.Thread(target=kafka_worker, args=(plotter,), daemon=True)
        _consumer_thread_ref = thread
        thread.start()

    # Register cleanup handler - will run before daemon threads are killed
    atexit.register(cleanup_consumer)

    # Material-ish dark theme + kill Plotly white backgrounds globally
    # NOTE: must not create UI elements in global scope when using @ui.page.
    # We'll enable dark mode inside the page function.

    ui.add_head_html(
        """
<style>
:root {
  --bg: #0b1020;
  --surface: rgba(255,255,255,0.05);
  --surface2: rgba(255,255,255,0.07);
  --border: rgba(255,255,255,0.10);
  --text: #e8edf7;
  --muted: rgba(232,237,247,0.72);
}

html, body {
  background: var(--bg) !important;
  color: var(--text) !important;
  overflow-x: hidden;
}
.q-page, .q-layout, .q-page-container { overflow-x: hidden; }

/* App layout spacing (Material dashboard-ish) */
.sky-container { max-width: 1200px; margin: 0 auto; padding: 16px; }
@media (max-width: 720px) { .sky-container { padding: 12px; } }

.sky-appbar {
  background: linear-gradient(135deg, rgba(79,195,247,0.18), rgba(124,77,255,0.10));
  border: 1px solid var(--border);
}

.sky-card {
  background: var(--surface);
  border: 1px solid rgba(255,255,255,0.08);
  border-radius: 16px;
}

/* Better readable text defaults */
.q-field__label, .q-field__native, .q-item__label, .q-tab__label { color: var(--text) !important; }
.q-field__hint, .q-field__bottom, .text-gray-300 { color: var(--muted) !important; }

/* Tabs (Material-ish) */
.q-tabs { background: rgba(255,255,255,0.04); border-radius: 14px; padding: 4px; }
.q-tab { border-radius: 12px; }
.q-tab--active { background: rgba(79,195,247,0.20) !important; }

/* Plotly: absolutely kill white backgrounds (paper + svg rects) */
.js-plotly-plot, .plotly, .plot-container, .svg-container, .main-svg {
  background: transparent !important;
}
.js-plotly-plot .bg { fill: rgba(0,0,0,0) !important; }
.js-plotly-plot .main-svg { background: transparent !important; }

/* Prevent Plotly from forcing ever-growing widths in flex layouts */
.js-plotly-plot, .plot-container, .svg-container, .plotly {
  width: 100% !important;
  max-width: 100% !important;
}

/* Plotly modebar */
.modebar { background: rgba(0,0,0,0.28) !important; }
.modebar-btn path { fill: rgba(232,237,247,0.75) !important; }
</style>
""",
        shared=True,
    )

    # Define the main page with @ui.page decorator
    @ui.page('/')
    async def index_page():
        import asyncio
        plotter._loop = asyncio.get_running_loop()

        # Material-ish dark theme (page-local)
        ui.dark_mode().enable()
        ui.colors(primary='#4FC3F7', secondary='#26A69A', accent='#7C4DFF', dark='#0B1020', positive='#21BA45', negative='#C10015', info='#31CCEC', warning='#F2C037')

        with ui.column().classes('w-full sky-container gap-4'):

            # Header / branding
            with ui.row().classes('w-full items-center justify-between sky-appbar px-4 py-3 rounded-2xl'):
                with ui.row().classes('items-center gap-4'):
                    ui.image(f'/assets/{RAVEN_IMG}').classes('h-11 w-11 object-cover rounded-xl')
                    with ui.column().classes('gap-0'):
                        ui.label('SKYTRACE').classes('text-xl font-semibold tracking-wide')
                        ui.label('RF SIGNAL INTELLIGENCE SUITE').classes('text-xs text-gray-300')
                with ui.column().classes('items-end gap-0'):
                    ui.label('EDGEHUB · LoRa Detector + Dechirp').classes('text-sm font-medium')
                    ui.label(
                        'LOCAL UI MODE'
                        if os.getenv('DISABLE_KAFKA','0') in ('1','true','TRUE','yes','YES')
                        else 'KAFKA CONNECTED'
                    ).classes('text-xs text-gray-300')

            with ui.tabs().classes('w-full') as tabs:
                tab_live = ui.tab('Live spectrum')
                tab_dechirp = ui.tab('Dechirp demo')

            with ui.tab_panels(tabs, value=tab_live).classes('w-full').props('keep-alive'):
                with ui.tab_panel(tab_live):
                    with ui.column().classes('w-full sky-card p-3 md:p-4'):
                        plotter.build_ui()

            with ui.tab_panel(tab_dechirp):
                ui.markdown(
                    """This demo collapses LoRa chirps into a narrow tone by **dechirping** using the detector's
estimated chirp slope. It's a visualization tool (no payload decode).

Requirements:
- ENABLE_DETECTOR=true (so we have chirp metrics)
- detector must emit controller fingerprints (edgehub-lora-detector)
"""
                )

                base_select = ui.select(options=[], label='Capture base_path (from Kafka cache)').classes('w-full')

                with ui.row().classes('w-full items-end'):
                    meta_in = ui.input(label='Or meta_file_path (local)', placeholder='/path/to/foo.sigmf-meta').classes('w-full')
                    data_in = ui.input(label='data_file_path (local)', placeholder='/path/to/foo.sigmf-data').classes('w-full')
                    ui.button('Run detector on paths', on_click=lambda: run_detector_on_paths()).props('outline')

                ctrl_select = ui.select(options=[], label='Controller').classes('w-full')

                # Two-up responsive row (side-by-side on md+, stacked on small screens)
                # Use min-w-0/overflow-hidden to stop Plotly from forcing width growth.
                with ui.row().classes('w-full gap-4 flex flex-col md:flex-row'):
                    fig_a = go.Figure(layout=go.Layout(template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'))
                    fig_b = go.Figure(layout=go.Layout(template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'))

                    plot_a = ui.plotly(fig_a).classes(
                        'w-full md:w-1/2 h-[24rem] md:h-[28rem] sky-card p-2 rounded-lg min-w-0 overflow-hidden'
                    )
                    plot_b = ui.plotly(fig_b).classes(
                        'w-full md:w-1/2 h-[24rem] md:h-[28rem] sky-card p-2 rounded-lg min-w-0 overflow-hidden'
                    )

                # Full-width plot beneath
                # Metrics + peak track
                with ui.row().classes('w-full gap-4 flex flex-col lg:flex-row'):
                    with ui.column().classes('w-full lg:w-1/3 sky-card p-4'):
                        ui.label('CFO / Drift Metrics').classes('text-sm font-medium')
                        ui.label('Derived from dechirped peak-frequency track (relative to controller center).').classes('text-xs text-gray-300')
                        cfo_kpi = ui.markdown('*(render to compute)*').classes('text-sm')

                        ui.separator()
                        ui.label('Symbol barcode').classes('text-sm font-medium')
                        ui.label('Piecewise-constant dechirped tone bins (fingerprint-friendly).').classes('text-xs text-gray-300')
                        sym_kpi = ui.markdown('*(render to compute)*').classes('text-sm')

                    fig_peak = go.Figure(layout=go.Layout(template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'))
                    peak_plot = ui.plotly(fig_peak).classes('w-full lg:w-2/3 h-64 sky-card p-2 rounded-lg min-w-0 overflow-hidden')

                fig_bar = go.Figure(layout=go.Layout(template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'))
                barcode_plot = ui.plotly(fig_bar).classes('w-full h-52 sky-card p-2 rounded-lg min-w-0 overflow-hidden')

                # Cached barcodes (from prior renders) for sexy comparisons.
                # Keep it server-global so it survives UI reloads/reconnects even if session storage resets.
                def _barcode_cache() -> dict:
                    return _BARCODE_CACHE

                def _barcode_cache_put(key: str, value: dict) -> None:
                    with _BARCODE_CACHE_LOCK:
                        _BARCODE_CACHE[key] = value

                def _barcode_cache_get() -> dict:
                    with _BARCODE_CACHE_LOCK:
                        return dict(_BARCODE_CACHE)

                with ui.column().classes('w-full sky-card p-4'):
                    ui.label('Barcode compare (fingerprint visual match)').classes('text-sm font-medium')
                    ui.label('Render two controllers, then compare their symbol barcodes.').classes('text-xs text-gray-300')

                    compare_a = ui.select(options={}, label='A (rendered controller)').classes('w-full')
                    compare_b = ui.select(options={}, label='B (rendered controller)').classes('w-full')
                    compare_kpi = ui.markdown('*(render two controllers first)*').classes('text-sm')

                    fig_cmp = go.Figure(layout=go.Layout(template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'))
                    cmp_plot = ui.plotly(fig_cmp).classes('w-full h-56 min-w-0 overflow-hidden')

                    def _refresh_compare_options():
                        cache = _barcode_cache_get()
                        opts = {k: k for k in cache.keys()}
                        compare_a.options = opts
                        compare_b.options = opts
                        if opts:
                            if compare_a.value not in opts.values():
                                compare_a.value = next(iter(opts.values()))
                            if compare_b.value not in opts.values():
                                compare_b.value = next(iter(opts.values()))
                        else:
                            compare_a.value = None
                            compare_b.value = None
                        compare_kpi.content = f"Cached barcodes: `{len(opts)}`"

                    def do_compare():
                        if barcode_similarity is None:
                            ui.notify('barcode_similarity not available', type='negative')
                            return
                        ka = compare_a.value
                        kb = compare_b.value
                        if not ka or not kb:
                            ui.notify('Select A and B', type='warning')
                            return
                        cache = _barcode_cache_get()
                        a = cache.get(ka) or {}
                        b = cache.get(kb) or {}
                        abins = (a.get('symbol_bins') or [])
                        bbins = (b.get('symbol_bins') or [])
                        sim = barcode_similarity(abins, bbins, max_shift=8) or {}
                        if not sim:
                            compare_kpi.content = '*(no barcode data for one or both selections)*'
                            return
                        compare_kpi.content = (
                            f"**Levenshtein sim:** `{sim.get('levenshtein_similarity',0.0):.3f}`  \n"
                            f"**Best-shift sim:** `{sim.get('best_shift_similarity',0.0):.3f}`  \n"
                            f"**Best shift:** `{sim.get('best_shift',0)}`  \n"
                            f"**Lens:** `{sim.get('len_a',0)}` vs `{sim.get('len_b',0)}`"
                        )

                        # Align by best shift for visualization
                        s = int(sim.get('best_shift', 0))
                        if s >= 0:
                            aa = abins[s:]
                            bb = bbins[: len(aa)]
                        else:
                            bb = bbins[-s:]
                            aa = abins[: len(bb)]
                        n = min(len(aa), len(bb))
                        aa = aa[:n]
                        bb = bb[:n]

                        # Build an overlay showing matches/mismatches
                        # z_val is the barcode; z_match is 1 for match, 0 for mismatch
                        z_val = [aa, bb]
                        z_match = [[1 if aa[i] == bb[i] else 0 for i in range(n)] for _ in range(2)]

                        fig = go.Figure(
                            layout=go.Layout(
                                template='plotly_dark',
                                paper_bgcolor='rgba(0,0,0,0)',
                                plot_bgcolor='rgba(0,0,0,0)',
                                title='Barcode compare (aligned; green=match, red=mismatch)',
                                xaxis_title='aligned symbol index',
                                yaxis=dict(
                                    tickmode='array',
                                    tickvals=[0, 1],
                                    ticktext=['A', 'B'],
                                ),
                                margin=dict(l=30, r=10, t=40, b=40),
                            )
                        )

                        # Base layer: turbo barcode values
                        fig.add_trace(
                            go.Heatmap(
                                z=z_val,
                                colorscale='Turbo',
                                showscale=False,
                                opacity=0.95,
                            )
                        )

                        # Overlay: match mask (green/red) with some transparency
                        fig.add_trace(
                            go.Heatmap(
                                z=z_match,
                                colorscale=[
                                    [0.0, 'rgba(255, 60, 60, 0.35)'],
                                    [0.4999, 'rgba(255, 60, 60, 0.35)'],
                                    [0.5, 'rgba(80, 255, 160, 0.28)'],
                                    [1.0, 'rgba(80, 255, 160, 0.28)'],
                                ],
                                showscale=False,
                            )
                        )

                        cmp_plot.update_figure(fig)

                    with ui.row().classes('w-full gap-2'):
                        ui.button('Compare (align + highlight)', on_click=do_compare).props('outline')

                        def clear_cache():
                            with _BARCODE_CACHE_LOCK:
                                _BARCODE_CACHE.clear()
                            _refresh_compare_options()
                            ui.notify('Barcode cache cleared', type='info')

                        ui.button('Clear cache', on_click=clear_cache).props('outline')

                    # Populate options once on page load
                    _refresh_compare_options()

                # Ambiguity-style map: show where dechirp alignment "locks".
                with ui.column().classes('w-full sky-card p-4'):
                    ui.label('Dechirp Ambiguity Map').classes('text-sm font-medium')
                    ui.label('Sweep slope/center offsets; the correct alignment should pop as a bright peak.').classes('text-xs text-gray-300')

                    fig_amb = go.Figure(layout=go.Layout(template='plotly_dark', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'))
                    amb_plot = ui.plotly(fig_amb).classes('w-full h-72 min-w-0 overflow-hidden')

                    async def render_ambiguity():
                        if ambiguity_map is None:
                            ui.notify('ambiguity_map not available', type='negative')
                            return
                        ev = _get_event_for_ui()
                        if not ev:
                            ui.notify('Select a base_path or provide local paths', type='warning')
                            return
                        cid = ctrl_select.value
                        if cid is None or cid == '':
                            ui.notify('Select a controller', type='warning')
                            return
                        if isinstance(ctrl_select.options, dict) and isinstance(cid, str):
                            cid_mapped = ctrl_select.options.get(cid)
                            if cid_mapped is not None:
                                cid = cid_mapped
                        try:
                            cid_int = int(cid)
                        except Exception:
                            cid_int = cid

                        det = (ev.get('detections') or {})
                        c = next((x for x in (det.get('controllers') or []) if x.get('controller_id') == cid_int), None)
                        if not c:
                            ui.notify('Controller not found', type='warning')
                            return
                        fp = c.get('fingerprint') or {}
                        slope = fp.get('chirp_slope_hz_per_s')
                        if slope is None:
                            ui.notify('No chirp slope available', type='warning')
                            return
                        tb = c.get('time_bounds_s') or {}
                        t_start = float(tb.get('start', 0.0))
                        t_end = float(tb.get('end', 0.0))
                        meta_path = (ev.get('artifact') or {}).get('meta_file_path')
                        data_path = (ev.get('artifact') or {}).get('data_file_path')
                        if not meta_path or not data_path:
                            ui.notify('Missing artifact paths', type='negative')
                            return
                        meta = load_sigmf_meta(meta_path)
                        _fs, fc_hz, *_ = parse_sigmf_params(meta)
                        fb = c.get('freq_bounds_hz') or {}
                        target_center_hz = float(fb.get('center', fc_hz))

                        notif = ui.notification('Computing ambiguity map…', timeout=None)
                        try:
                            def compute_map():
                                return ambiguity_map(
                                    meta_path=meta_path,
                                    data_path=data_path,
                                    t_start_s=t_start,
                                    t_end_s=t_end,
                                    capture_center_hz=float(fc_hz),
                                    target_center_hz=float(target_center_hz),
                                    slope_hz_per_s=float(slope),
                                    pad_s=0.03,
                                    nfft=1024,
                                    hop=256,
                                )
                            m = await run.io_bound(compute_map)
                        except Exception as e:
                            ui.notify(f'ambiguity failed: {e}', type='negative')
                            return
                        finally:
                            notif.dismiss()

                        y = m.get('freq_offsets_hz') or []
                        x = m.get('slope_offsets_hz_per_s') or []
                        z = m.get('tone_score') or []
                        fig = go.Figure(
                            data=go.Heatmap(
                                x=[v/1e6 for v in x],
                                y=y,
                                z=z,
                                colorscale='Viridis',
                                colorbar=dict(title='tone_score'),
                            ),
                            layout=go.Layout(
                                template='plotly_dark',
                                paper_bgcolor='rgba(0,0,0,0)',
                                plot_bgcolor='rgba(0,0,0,0)',
                                title='Ambiguity map (tone_score)',
                                xaxis_title='slope offset (MHz/s)',
                                yaxis_title='freq offset (Hz)',
                                margin=dict(l=50, r=10, t=40, b=45),
                            ),
                        )
                        amb_plot.update_figure(fig)

                    ui.button('Render ambiguity map', on_click=render_ambiguity).props('outline')

                def refresh_bases():
                    with _latest_det_lock:
                        bases = sorted(_latest_det_by_base.keys())
                    base_select.options = bases
                    if bases and (base_select.value not in bases):
                        base_select.value = bases[-1]

                def _get_event_for_ui():
                    # Prefer explicit paths (local test mode). Otherwise use Kafka cache.
                    mp = (meta_in.value or '').strip()
                    dp = (data_in.value or '').strip()
                    if mp and dp:
                        base_path = mp[:-11] if mp.endswith('.sigmf-meta') else mp
                        return {
                            'artifact': {
                                'base_path': base_path,
                                'meta_file_path': mp,
                                'data_file_path': dp,
                            },
                            'detections': _local_det_cache.get('detections') if _local_det_cache else None,
                        }

                    b = base_select.value
                    if not b:
                        return None
                    with _latest_det_lock:
                        return _latest_det_by_base.get(b)

                def refresh_controllers():
                    ev = _get_event_for_ui()
                    det = (ev or {}).get('detections') or {}
                    ctrls = det.get('controllers') or []

                    # NiceGUI select: dict options show keys and return values.
                    opts = {}
                    for c in ctrls:
                        cid = c.get('controller_id')
                        fp = c.get('fingerprint') or {}
                        bins = fp.get('bins') or {}
                        label = f"controller_id={cid}  fp={c.get('fingerprint_id','')}  center={bins.get('center_hz','?')}  bw={bins.get('bandwidth_hz','?')}"
                        opts[label] = cid

                    ctrl_select.options = opts
                    try:
                        ctrl_select.update()
                    except Exception:
                        pass

                    if opts and (ctrl_select.value not in set(opts.values())):
                        # pick first value
                        ctrl_select.value = next(iter(opts.values()))

                def on_base_change(_):
                    # Clear local path mode when selecting from cache
                    refresh_controllers()

                base_select.on('update:model-value', on_base_change)

                _local_det_cache = {}

                def run_detector_on_paths():
                    mp = (meta_in.value or '').strip()
                    dp = (data_in.value or '').strip()
                    if not mp or not dp:
                        ui.notify('Provide meta_file_path and data_file_path', type='warning')
                        return
                    try:
                        d = run_detector(mp, dp)
                    except Exception as e:
                        ui.notify(f'Detector failed: {e}', type='negative')
                        return
                    _local_det_cache['detections'] = d
                    n_ctrl = len((d.get('controllers') or []))
                    n_det = len((d.get('detections') or []))
                    ui.notify(f'Detector OK; controllers={n_ctrl} detections={n_det}', type='positive')
                    refresh_controllers()

                async def render():
                    if make_dechirp_waterfall is None or make_shifted_waterfall is None:
                        ui.notify('dechirp_tools not available in this drop-in', type='negative')
                        return

                    ev = _get_event_for_ui()
                    if not ev:
                        ui.notify('Select a base_path or provide local paths', type='warning')
                        return

                    cid = ctrl_select.value
                    if cid is None or cid == '':
                        ui.notify('Select a controller', type='warning')
                        return

                    # NiceGUI/Quasar sometimes returns the option *label*.
                    # If so, map label -> id using the options dict.
                    if isinstance(ctrl_select.options, dict) and isinstance(cid, str):
                        cid_mapped = ctrl_select.options.get(cid)
                        if cid_mapped is not None:
                            cid = cid_mapped

                    try:
                        cid_int = int(cid)
                    except Exception:
                        cid_int = cid

                    det = ev.get('detections') or {}
                    ctrls = det.get('controllers') or []
                    available_ids = [x.get('controller_id') for x in ctrls]
                    c = next((x for x in ctrls if x.get('controller_id') == cid_int), None)
                    if c is None:
                        ui.notify(
                            f'Controller not found: selected={cid_int} available={available_ids}',
                            type='negative',
                        )
                        return

                    fp = c.get('fingerprint') or {}
                    slope = fp.get('chirp_slope_hz_per_s')
                    if slope is None:
                        ui.notify('No chirp slope available (try another controller/capture)', type='warning')
                        return

                    tb = c.get('time_bounds_s') or {}
                    t_start = float(tb.get('start', 0.0))
                    t_end = float(tb.get('end', 0.0))

                    meta_path = (ev.get('artifact') or {}).get('meta_file_path')
                    data_path = (ev.get('artifact') or {}).get('data_file_path')
                    if not meta_path or not data_path:
                        ui.notify('Missing artifact paths', type='negative')
                        return

                    meta = load_sigmf_meta(meta_path)
                    _fs_hz, fc_hz, *_ = parse_sigmf_params(meta)

                    fb = c.get('freq_bounds_hz') or {}
                    target_center_hz = float(fb.get('center', fc_hz))

                    # Heavy compute: run off the UI thread to avoid websocket timeouts / reloads.
                    def compute():
                        tt0, ff0, p0, pkf0, _pkdb0 = make_shifted_waterfall(
                            meta_path=meta_path,
                            data_path=data_path,
                            t_start_s=t_start,
                            t_end_s=t_end,
                            capture_center_hz=float(fc_hz),
                            target_center_hz=float(target_center_hz),
                            pad_s=0.05,
                            nfft=2048,
                            hop=512,
                        )
                        tt1, ff1, p1, pkf1, _pkdb1 = make_dechirp_waterfall(
                            meta_path=meta_path,
                            data_path=data_path,
                            t_start_s=t_start,
                            t_end_s=t_end,
                            capture_center_hz=float(fc_hz),
                            target_center_hz=float(target_center_hz),
                            slope_hz_per_s=float(slope),
                            pad_s=0.05,
                            nfft=2048,
                            hop=512,
                        )
                        return tt0, ff0, p0, pkf0, tt1, ff1, p1, pkf1

                    notif = ui.notification('Rendering dechirp…', timeout=None)
                    try:
                        tt0, ff0, p0, pkf0, tt1, ff1, p1, pkf1 = await run.io_bound(compute)
                    except Exception as e:
                        notif.dismiss()
                        ui.notify(f'render failed: {e}', type='negative')
                        return
                    finally:
                        notif.dismiss()

                    fig0 = go.Figure(
                        data=go.Heatmap(
                            x=tt0,
                            y=ff0 / 1e3,
                            z=p0.T,
                            colorscale='Viridis',
                            zsmooth='best',
                            colorbar=dict(title='dB'),
                        ),
                        layout=go.Layout(
                            template='plotly_dark',
                            paper_bgcolor='rgba(0,0,0,0)',
                            plot_bgcolor='rgba(0,0,0,0)',
                            title='Shifted to controller center (raw chirps)',
                            xaxis_title='time (s)',
                            yaxis_title='freq (kHz, relative)',
                            margin=dict(l=40, r=10, t=40, b=40),
                        ),
                    )
                    fig1 = go.Figure(
                        data=go.Heatmap(
                            x=tt1,
                            y=ff1 / 1e3,
                            z=p1.T,
                            colorscale='Viridis',
                            zsmooth='best',
                            colorbar=dict(title='dB'),
                        ),
                        layout=go.Layout(
                            template='plotly_dark',
                            paper_bgcolor='rgba(0,0,0,0)',
                            plot_bgcolor='rgba(0,0,0,0)',
                            title='Shifted + dechirped (chirp collapses to tone)',
                            xaxis_title='time (s)',
                            yaxis_title='freq (kHz, relative)',
                            margin=dict(l=40, r=10, t=40, b=40),
                        ),
                    )
                    plot_a.update_figure(fig0)
                    plot_b.update_figure(fig1)

                    # CFO / drift metrics from dechirped peak track
                    if cfo_metrics is not None:
                        m = cfo_metrics(tt1, pkf1)
                        abs_center_est = float(target_center_hz) + float(m.get('offset_median_hz', 0.0))
                        cfo_kpi.content = (
                            f"**Offset (median):** `{m.get('offset_median_hz', 0.0):.1f} Hz`  \n"
                            f"**Offset (std):** `{m.get('offset_std_hz', 0.0):.1f} Hz`  \n"
                            f"**Drift:** `{m.get('drift_hz_per_s', 0.0):.2f} Hz/s`  \n"
                            f"**Tone score:** `{m.get('tone_score', 0.0):.3f}`  \n"
                            f"**Estimated carrier:** `{abs_center_est/1e6:.6f} MHz`  \n"
                            f"**Samples:** `{m.get('samples', 0)}`"
                        )

                    # Symbol barcode (tone-bin sequence) from dechirped peak track
                    if symbol_barcode_metrics is not None:
                        # df from STFT bin spacing
                        df_hz = float((ff1[1] - ff1[0]) if (ff1 is not None and len(ff1) > 1) else 0.0)
                        sb = symbol_barcode_metrics(tt1, pkf1, df_hz=df_hz)
                        if sb:
                            sym_kpi.content = (
                                f"**Symbols:** `{sb.get('segments', 0)}`  \n"
                                f"**Symbol period (est):** `{sb.get('symbol_period_s', 0.0)*1e3:.2f} ms`  \n"
                                f"**Confidence:** `{sb.get('confidence', 0.0):.3f}`"
                            )

                            # Visual barcode (single-row heatmap)
                            bins = sb.get('symbol_bins') or []
                            z = [bins]  # one row
                            figb = go.Figure(
                                data=go.Heatmap(
                                    z=z,
                                    colorscale='Turbo',
                                    showscale=False,
                                ),
                                layout=go.Layout(
                                    template='plotly_dark',
                                    paper_bgcolor='rgba(0,0,0,0)',
                                    plot_bgcolor='rgba(0,0,0,0)',
                                    title='Symbol barcode (tone bin index per symbol)',
                                    xaxis_title='symbol index',
                                    yaxis=dict(showticklabels=False),
                                    margin=dict(l=20, r=10, t=40, b=40),
                                    height=180,
                                ),
                            )
                            barcode_plot.update_figure(figb)

                            # Cache for compare
                            label = f"{(ev.get('artifact') or {}).get('base_path','?')} :: controller_id={cid_int}"
                            _barcode_cache_put(
                                label,
                                {
                                    'symbol_bins': bins,
                                    'confidence': float(sb.get('confidence', 0.0)),
                                    'symbol_period_s': float(sb.get('symbol_period_s', 0.0)),
                                },
                            )
                            _refresh_compare_options()
                        else:
                            sym_kpi.content = '*(barcode unavailable: insufficient symbol segments)*'
                            # Keep compare options stable even if this render couldn't extract a barcode
                            _refresh_compare_options()

                    figp = go.Figure(
                        data=[
                            go.Scatter(x=tt0, y=pkf0 / 1e3, mode='lines', name='peak freq (raw shifted)'),
                            go.Scatter(x=tt1, y=pkf1 / 1e3, mode='lines', name='peak freq (dechirped)'),
                        ],
                        layout=go.Layout(
                            template='plotly_dark',
                            paper_bgcolor='rgba(0,0,0,0)',
                            plot_bgcolor='rgba(0,0,0,0)',
                            title='Peak frequency track (CFO/drift evidence)',
                            xaxis_title='time (s)',
                            yaxis_title='peak freq (kHz, relative)',
                            margin=dict(l=40, r=10, t=40, b=40),
                        ),
                    )
                    peak_plot.update_figure(figp)

                with ui.row().classes('w-full'):
                    ui.button('Refresh list', on_click=lambda: (refresh_bases(), refresh_controllers()))
                    ui.button('Render side-by-side + peak track', on_click=render)

                # Initial population
                refresh_bases()
                refresh_controllers()
    
    # Start the UI (The "Blocking" call)
    # Use reload=False for stability in containerized/threaded setups
    # show=False prevents browser auto-open in container
    ui.run(
        port=settings.plotter.port,
        reload=False,
        show=False,
        # This tells uvicorn to handle signals properly
        uvicorn_logging_level='info'
    )


if __name__ in {"__main__", "__mp_main__"}:
    run_local_consumer()
